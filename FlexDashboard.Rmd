...
```{r}
data <- read.csv("F:/Data Analytics/Data Sources/vehicle.csv")
```
...
```{r}
p1<-data %>% group_by(State)%>% summarise(count =n())%>% plot_ly(x=~State,
    y=~count,color = "blue",type = 'bar')%>% 
...
```{r}
p2 <- data %>%
         group_by(State)%>% summarise(count=n())%>% filter(count>50)%>%
  plot_ly(labels=~State,
          values = ~count,
          marker=list(color=mycolors))%>% add_pie(hole=0.2)%>%layout(xaxis=list(zaroline=F,showline=F,showticklabels=F,
showgrid=F),yaxis=list(zaroline=F,showline=F,showticklabels=F,
showgrid=F))
p2
```
...
p4 <- plot_ly(data, x=~fm) %>%
        add_markers(y=~Mileage,text=~paste("Mileage:",Mileage),showlegend=F)%>%
  add_lines(y=~fitted(loess(Mileage~fm)),name="loses Smother",color=I("#FFC125"),showlegend=T,line=list(width=5))%>%layout(xaxis=list(title="Month"),yaxis=list(title="Milege"))
p4
```
...
```{r}
data%>% group_by(State)%>%ggvis(~State,~lc,fill=~State)%>% layer_boxplots()
```
...
Created by: Muhammad Ehsan a Data Analyst
